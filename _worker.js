...
let subconfig = "https://raw.githubusercontent.com/PingshaZoo/cfvless/refs/heads/main/ACL4SSR_Online_Mini_MultiMode.ini"; //订阅配置文件
...
export default {
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            const timestamp = Math.ceil(currentDateTime.getTime() / (10 * 10 * 10));
            const fakeUserIDMD5 = await MD5MD5(`${userID}${timestamp}`);
            fakeUserID = fakeUserIDMD5.slice(0, 8) + "-" + fakeUserIDMD5.slice(8, 12) + "-" + fakeUserIDMD5.slice(12, 16) + "-" + fakeUserIDMD5.slice(16, 20) + "-" + fakeUserIDMD5.slice(20);

            fakeHostName = fakeUserIDMD5.slice(6, 9);
            fakeHostName = fakeHostName + "." + fakeUserIDMD5.slice(13, 19);
...
async function vlessOverWSHandler(request) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        ...
        abort(reason) {
            log(`readableWebSocketStream is terminated`, JSON.stringify(reason));
        },
...
function makeReadableWebSocketStream(webSocketServer, earlyDataHeader, log) {
    ...
    const stream = new ReadableStream({
        ...
        cancel(reason) {
            ...
            log(` readable stream is canceled，reason is : ${reason}`);
...
async function handleDNSQuery(udpChunk, webSocket, vlessResponseHeader, log) {
    ...
    try {
        ...
        const dnsServer = '4.2.2.1'; // 在 Cloudflare 修复连接自身 IP 的 bug 后，将改为 1.1.1.1
        ...
        // 捕获并记录任何可能发生的错误
        console.error(
            `handleDNSQuery function error，err info : ${error.message}`
        );
    }
}
